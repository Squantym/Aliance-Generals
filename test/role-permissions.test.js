...
ok(v0.roles.every((r) => r.zones.length === 0), 'и так у всех трёх ролей');
// Роль «арбитр» упразднена: настраивать осталось администратора,
// комиссара и дозор. Владельца в списке нет — у него всегда полный
// доступ, и отключить себе раздел он не может.
ok(v0.roles.length === 3, `настраиваются: ${v0.roles.map((r) => r.name).join(', ')}`);
