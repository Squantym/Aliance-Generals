...
// Этому тесту мало якоря: он кладёт временный файл в public/img/bosses/
// и просит СЕРВЕРНЫЙ модуль его перечислить, а тот ищет папку от текущей
// каталога — как и на боевом, где pm2 запускает игру из корня установки.
// Без перехода тест сравнивал бы файл в проекте со списком из /tmp.
// Данные это не трогает: пишется и тут же удаляется один файл картинки.
process.chdir(ROOT);
let passed = 0;
