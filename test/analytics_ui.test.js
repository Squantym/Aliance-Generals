...
console.log('\n── Воронка: верхние уровни и иконки валют ──');
// ВНИМАНИЕ: в этом файле ok(название, условие) — обратный порядок
// относительно других тестов. Сначала я написал ok(условие, название):
// строка-название всегда истинна, поэтому ВСЕ девять проверок проходили,
// ничего не проверяя. Ровно тот случай, когда зелёная галочка вреднее
// красной. Порядок ниже правильный.
const PROJ = require('path').join(__dirname, '..');
const admSrcF = fs.readFileSync(PROJ + '/public/js/admin.js', 'utf8');
const anaSrc = fs.readFileSync(PROJ + '/src/services/analytics.ts', 'utf8');
// Ступени до 200-го уровня добавлены по просьбе владельца: раньше вся
// вторая половина игры была одной строкой, и по воронке нельзя было
// понять, где упирается долгожитель.
for (const n of [50, 100, 150, 200]) {
  ok(`в воронке есть ступень «${n} уровня»`, new RegExp(`Дошли до ${n} уровня`).test(anaSrc));
}
// Иконки валют — те же картинки, что в игре, а не эмодзи.
ok('у панели есть иконка золота из игры', /ICG: '<span class="ic-gold"><\/span>'/.test(admSrcF));
ok('и иконка долларов', /ICD: '<span class="ic-dollar"><\/span>'/.test(admSrcF));
ok('иконки действительно подставляются в разметку',
   /\$\{Admin\.ICG\}/.test(admSrcF) && /\$\{Admin\.ICD\}/.test(admSrcF));
// Считаем только разметку: в комментариях эмодзи объясняет само себя,
// в уведомлениях (UI.toast) разметки нет вовсе, а data-l="🪙" — подпись
// столбца для узкого экрана, она выводится через CSS content и картинку
// туда не подставить.
const markupOnly = admSrcF
  .replace(/(^|\n)\s*\/\/[^\n]*/g, '$1')        // комментарии
  .replace(/UI\.toast\([^;]*\);/g, '')            // уведомления
  .replace(/data-l="[^"]*"/g, '');                 // подписи столбцов
const emojiLeft = (markupOnly.match(/🪙|💵/g) || []).length;
ok(`в разметке панели эмодзи валют не осталось (найдено ${emojiLeft})`, emojiLeft === 0);
// Липкость: поле может не прийти — «undefined%» на экране недопустимо
ok('отсутствующая липкость показывается прочерком, а не «undefined%»',
   /a\.stickiness == null \? '—'/.test(admSrcF));
