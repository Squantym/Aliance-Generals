...
console.log('\n[3] Рейтинг за срыв герба: +3 нападавшему, −3 хозяину');
// Раздел проверял двойной срез трофея «Тесак мясника»: за второе ухо
// рейтинг начислялся ДВАЖДЫ. Сам трофей из игры убран, но правило
// «одна часть — три очка обеим сторонам» осталось, и стеречь его надо.
reset();
const B2 = mk('a'); const V2 = mk('v');
um['a'] = B2; um['v'] = V2;
B2.pendingBreach = { targetId: 'v', isBot: false, exp: Date.now() + 60000 };
battle.breach(B2, 'crest', []);
eq('сорвана одна часть', V2.crestPartsLost, 1);
eq('рейтинг нападавшего +3', B2.rating, 3);
eq('рейтинг хозяина штаба −3', V2.rating, -3);
