...
console.log('\n[2] Исход боя — по МОЩИ (сильнейший побеждает), уворот НЕ переворачивает');
{
  ...
  // Правило: побеждает тот, кто нанёс больше урона; но при УВОРОТЕ исход
  // решает реальная мощь — увернувшаяся цель больше не «переворачивает» бой.
  ok('сильный побеждает во всех боях (уворот не спасает слабого)', strongWins === 400);
  ...
  // Новое правило: при увороте исход решает РЕАЛЬНАЯ МОЩЬ. Слабый атакующий
  // с максимальной ловкостью больше НЕ выигрывает у сильного: он не получает
  // урона, но и победу это ему не приносит.
  console.log(`     слабый с ловкостью 100 выигрывает в ${(weakWins/400*100).toFixed(1)}% боёв`);
  ok('свой уворот больше НЕ приносит победу слабому', weakWins === 0);
  ok('но уворот срабатывал (урон обнулялся)', atkDodges > 0);
