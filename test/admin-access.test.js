...
ok(/res\.writeHead\(404[\s\S]{0,160}Not found/.test(http), 'при заданном ADMIN_PATH стандартные адреса отвечают 404');
ok(!/if \(rel === '\/admin'\) rel = '\/admin\.html';/.test(http), 'прежняя прямая отдача панели убрана');
ok(/ADMIN_PATH && \(rel === ADMIN_PATH/.test(http), 'панель открывается по заданному секретному адресу');
// Требовать настройки ADMIN_PATH нельзя: без неё владелец терял доступ
// после переустановки. Когда путь не задан — работает привычный /admin,
// а права по-прежнему проверяются на каждом запросе.
ok(/} else if \(!ADMIN_PATH && \(rel === '\/admin' \|\| rel === '\/admin\/'\)\)/.test(http),
   'без настройки ADMIN_PATH работает обычный /admin');
// Панель ровно одна — v2. По адресу панели отдаётся она, а файла
// старой (admin.html) больше нет вовсе.
ok(!fs.existsSync(ROOT + '/public/admin.html'), 'страницы старой панели в проекте не осталось');
ok(!/rel = '\/admin\.html';/.test(http), 'по адресу панели отдаётся только v2');
ok(/ADMIN_PATH[\s\S]{0,400}Секретный путь не настроен/.test(http), 'в коде объяснено, когда какой адрес действует');
