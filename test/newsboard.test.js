...
(async () => {
  ...
  ok('перед отправкой картинка ужимается', /_prepareImage/.test(newsJs));
  const appJs = read('public/js/app.js');
  ok('подготовка живёт в ядре — она нужна двум экранам', /App\._prepareImage = /.test(appJs));
  ok('в «Общении» своей копии не осталось', !/App\._prepareImage = /.test(socialJs));
