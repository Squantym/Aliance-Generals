...
(async () => {
  ...
  // \b в JS считает словом только латиницу: с кириллицей он не срабатывал
  // НИКОГДА, и запрет молча пропускал любое «ухо» — ровно та пустая
  // проверка, от которой предостерегает CLAUDE.md. Границу слова задаём
  // сами, просмотром вперёд и назад по кириллице, иначе «воздуха» и
  // «лучший» ловились бы вместе с «ухом».
  const BAD = /fatalit|фаталит|(?<![А-Яа-яЁё])(уши|ушей|ухо|уха|ухом|ушам|ушами|ушко|\u{1F442})(?![А-Яа-яЁё])/iu;
  ...
  ok('и ловит кириллицу — раньше не ловил',
     BAD.test('Ухо восстановлено') && BAD.test('нужно 5 ушей') && BAD.test('иконка 👂'));
  ok('но не задевает слова, внутри которых те же буквы',
     !BAD.test('атака воздуха') && !BAD.test('лучший из лучших') && !BAD.test('слуха не было'));
