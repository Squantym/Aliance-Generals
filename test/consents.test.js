...
(async () => {
  ...
  console.log('\n── 8. Кнопки выгрузки данных у игрока больше нет ──');
  // Право на копию данных исполняется по запросу (Политика, п. 11.1):
  // выгрузку готовит сотрудник из карточки игрока. Игроку адрес закрыт.
  const mine = await get('/api/my-data', tok);
  ok('адрес самостоятельной выгрузки убран', mine.status === 404);
  const staffOnly = await get('/api/admin/player-data/' + encodeURIComponent(myId), tok);
  ok('служебная выгрузка игроку недоступна', staffOnly.status >= 400);
