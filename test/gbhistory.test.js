...
console.log('\n── 5. Победа считается ОДНИМ способом ──');
// Флаг iWon задавался в двух местах по-разному: в разборе сохранённого
// боя — по строке результата (mineRow.won), а в живом состоянии — по
// команде (winnerTeam === me.team). Для обычного игрока они совпадают,
// а не явившемуся из ПОБЕДИВШЕЙ команды один экран говорил «ПОБЕДА»,
// другой «ПОРАЖЕНИЕ». Неявка = поражение, это правило игры: за него
// играет его копия, награды нет.
const iWonLines = srv.split('\n').filter((l) => /iWon\s*:/.test(l));
ok(iWonLines.length >= 2, `определений iWon в сервере: ${iWonLines.length}`);
ok(iWonLines.every((l) => /forfeited/.test(l) || /mineRow\.won/.test(l)),
   'каждое определение iWon учитывает неявку');
ok(/forfeited:/.test(srv), 'сервер отдаёт признак неявки и в разбор сохранённого боя');

console.log('\n── 6. Неявку экран называет своим именем ──');
// Голое «ПОРАЖЕНИЕ» при победе своей команды человек читает как ошибку
// игры, а не как последствие своего опоздания.
ok(/НЕ ЯВИЛСЯ/.test(war), 'экран результата отдельно называет неявку');
ok(/b\.forfeited/.test(war), 'и берёт признак с сервера, а не угадывает');
ok(/30 секунд/.test(war), 'и объясняет, что именно было пропущено');
