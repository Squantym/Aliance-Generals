...
// Приём платежей запущен: документ, обещающий, что оплаты ещё нет, спорил
// бы с фактом списания денег
ok(!/Платежи пока не запущены/.test(payDoc) && !/приём платежей в Игре не запущен/.test(terms)
   && !/Приём платежей подключается/.test(payDoc) && !/Приём платежей подключается/.test(terms),
   'плашек «не запущены» и «подключается» больше нет — оплата работает');
ok(/Система быстрых платежей \(СБП\)/.test(payDoc) && /SberPay/.test(payDoc) && /T-Pay/.test(payDoc),
   'способы оплаты перечислены в Правилах платежей');
// Касса — Робокасса (с 273). Зарубежные карты владелец назвал критически
// важными: покупатель из-за рубежа должен видеть в оферте, что его карту
// принимают, а упоминание отключённой ЮKassa путало бы при споре с банком.
ok(/Робокасса/.test(payFlat) && /банковские карты<\/b>, в том числе выпущенные за пределами России/.test(payFlat) && !/ЮKassa/.test(payDoc),
   'в Правилах платежей — Робокасса и зарубежные карты, ЮKassa не упоминается');
