...
async function bootstrap(adminName, adminPass, adminEmail) {
  ...
  // Ждём отложенную запись на диск: сервер сбрасывает данные через
  // 400 мс, и остановка раньше срока оставляла базу без только что
  // заведённого игрока. На Linux это добирало финальное сохранение по
  // SIGTERM, на Windows kill() обработчик не запускает вовсе — и
  // grant-admin открывал пустую базу.
...
async function main() {
  ...
  console.log('0б. Версии файлов: правка экрана должна доходить до игрока');
  // Экраны грузятся из кода, и версию к адресу дописывает клиент. Раньше
  // он дописывал всем один хэш — от app.js, — и правка экрана не доходила
  // до игрока, пока не менялся app.js: такие адреса отдаются как
  // immutable на год.
  //
  // Свой сервер этот тест поднимает во временной папке, где нет public/,
  // и статику не отдаёт вовсе. Поэтому здесь проверяем только когда тест
  // направлен на НАСТОЯЩИЙ сервер (BASE извне). Разбор самой цепочки —
  // в test/assetver.test.js, он поднимает сервер из корня проекта.
  {
    const root = await fetch(BASE + "/").catch(() => null);
    const html = root && root.status === 200 ? await root.text() : null;
    if (!html) {
      skip('карта версий экранов в странице', 'свой сервер поднят без public/ — статики нет');
    } else {
      const m = /window\.__SCREENS=(\{.*?\});/.exec(html);
      check('сервер положил карту версий экранов в страницу', !!m,
            'её нет — экраны получат чужую версию и правки не дойдут');
      const appTag = /\/js\/app\.js\?v=([0-9a-f]{8})/.exec(html);
      let mapv = null;
      if (m) { try { mapv = JSON.parse(m[1]); } catch (e) {} }
      check('версия экрана не равна версии app.js',
            !!mapv && !!appTag && mapv.market !== appTag[1],
            'обе ' + (appTag && appTag[1]));
    }
  }
  ...
  // Клуб переписан не раз, и каждый раз убранная игра оставляла хвост:
  // тест тянул старый API и падал на первой же строке, скрывая всё, что
  // идёт ниже по сценарию. Сейчас в клубе очко, общий сейф, дуэль,
  // напёрстки, две игры с живой очередью и «Военный займ».
  const club = (await get('/api/club', A)).data;
  check('клуб отдаёт свои игры',
        !!club.pref && !!club.safe && !!club.tactic && !!club.thimble
        && !!club.intercept && !!club.sniper,
  ...
  const botGuard = await post('/api/club/thimble/play', A, { pot: 0 });
