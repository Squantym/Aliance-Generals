...
// Ступени ищем ПО НАЗВАНИЮ: воронку продлили до 200-го уровня, и
// нумерация сдвинулась. Проверка по индексу ломалась бы при каждом
// добавлении ступени — то есть при обычной правке аналитики.
const stepOf = (part) => f.find((x) => x.name.includes(part)) || {};
ok(stepOf('5 уровня').count === 3, `5-й уровень: ${stepOf('5 уровня').count}`);
ok(stepOf('10 уровня').count === 2, `10-й уровень: ${stepOf('10 уровня').count}`);
ok(stepOf('25 уровня').count === 1, `25-й уровень: ${stepOf('25 уровня').count}`);
ok(stepOf('легион').count === 1, `в легионе: ${stepOf('легион').count}`);
// Верхние ступени добавлены по просьбе владельца: без них вся вторая
// половина игры была одной строкой.
for (const n of [50, 100, 150, 200]) {
  ok(!!stepOf(`${n} уровня`), `есть ступень «${n} уровня»`);
}
ok(f.every((x, i, arr) => i === 0 || arr[i - 1].count >= x.count || x.name.includes('легион')),
   'уровневые ступени не растут — воронка сужается, как и положено');
let monotone = true;
