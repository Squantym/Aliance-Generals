...
// Раньше здесь проверялась запись в root2.personal и точное выражение
// «h.won ? 'Победа' : …». Это закрепляло ОШИБКУ: историю писали две
// разные функции в двух форматах, экран читал формат одной, а получал
// данные другой — и каждый бой показывался поражением. Тест при этом
// был зелёным, потому что сверял текст кода с самим собой.
//
// Теперь проверяем то, что видит игрок: писатель один, поле исхода одно
// и экран читает именно его.
ok(/root2\.playerHistory/.test(gbSrcH), 'личная история сохраняется');
ok(!/root2\.personal\[/.test(gbSrcH), 'второго писателя истории в другом формате не осталось');
ok(/myHistory:/.test(gbSrcH), 'и отдаётся на экран');
ok(/playerHistory \|\| \{\}\)\[user\.id\]/.test(gbSrcH), 'на экран отдаётся именно та коллекция, в которую пишут');
ok(/Ваши последние бои/.test(warR3), 'блок истории есть внизу');
ok(/h\.result === 'win'/.test(warR3) && /h\.result === 'draw'/.test(warR3),
   'исход боя определяется по полю result, которое и приходит с сервера');
