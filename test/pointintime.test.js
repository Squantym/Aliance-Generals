...
ok(/canAccessZone\(req\.user, 'database'\)/.test(rt.slice(hIdx, hIdx + 300)),
   'история закрыта владельческой зоной «База данных»');
const aj = fs.readFileSync(path.join(ROOT, 'public/js/admin.js'), 'utf8');
ok(/_historyHtml\(\)/.test(aj), 'блок истории есть в панели');
ok(/Admin\._historyHtml\(\)/.test(aj), 'и подключён к блоку базы');
