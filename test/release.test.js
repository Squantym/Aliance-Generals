...
(async () => {
  ...
  // раньше. На Linux финальное сохранение по SIGTERM всё добирало, на
  // Windows kill() обработчик не запускает вовсе, и grant-admin
  // открывал ПУСТУЮ базу: «игроков: 0, игрок не найден».
  ...
  // Состояние режима тоже пишется отложенно (400 мс). Останавливая
  // сервер раньше, тест проверял бы не «пережило перезапуск», а
  // «успело записаться» — и на Windows, где финального сохранения
  // нет вовсе, стабильно падал.
  await new Promise((r) => setTimeout(r, 900));
  await stop(srv);
  srv = await startServer(env);
