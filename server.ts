...
async function main() {
  ...
  // ---- Разовая миграция: боты вон из личных альянсов ----
  // Ботов набирали старой кнопкой «позвать бойца». Кнопку убрали в 217-м,
  // но набранные остались и продолжали давать вместимость армии — по 10
  // единиц техники за каждого, то есть мощь из ниоткуда. Чистим ОДИН раз
  // по флагу: это правка данных, повторять её при каждом старте нельзя —
  // живых союзников она не трогает, но лишний проход по всем игрокам
  // ничего не даёт, а полное сохранение базы стоит дорого.
  try {
    const meta = db.load<Record<string, any>>('meta', {});
    if (!meta.allianceBotsPurged) {
      const r = require('./src/services/personalAlliance').purgeBots();
      meta.allianceBotsPurged = Date.now();
      db.save('meta');
      if (r.removed) {
        db.save('users');   // полное сохранение: затронуты многие игроки
        console.log(`🧹 Личные альянсы: убрано ботов ${r.removed} у ${r.players} игроков.`);
      } else {
        console.log('🧹 Личные альянсы: ботов не найдено.');
      }
    }
  } catch (e) {
  ...
  // ---- Разовая миграция: односторонние союзы ----
  // Исключение убирало запись только у одной стороны, а приём заявки при
  // полном альянсе добавлял только второму. Такие пары игра союзниками
  // не считает — отсюда «он у меня в альянсе, а игра предлагает его
  // пригласить». Делаем взаимными (решение владельца 18.09.2026).
  try {
    const meta = db.load<Record<string, any>>('meta', {});
    if (!meta.allianceOneSidedFixed) {
      const r = require('./src/services/personalAlliance').repairOneSided();
      meta.allianceOneSidedFixed = Date.now();
      db.save('meta');
      if (r.pairs) {
        db.save('users');
        console.log(`🤝 Личные альянсы: восстановлено ${r.pairs} односторонних связей у ${r.players} игроков.`);
      } else {
        console.log('🤝 Личные альянсы: односторонних связей не найдено.');
      }
    }
  } catch (e) {
    console.error('Ошибка починки односторонних союзов:', e);
  }
