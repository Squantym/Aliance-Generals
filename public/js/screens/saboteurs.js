...
// типу), секретные (мощь секретных разработок), подрывники (мощь
...
App.screens.saboteurs = async (c, param) => {
  ...
  const tabsHtml = `<div class="tabs sab-tabs" style="flex-wrap:wrap">${tabs.map(([id, label]) =>
    `<div class="tab sab-tab ${id === tab ? 'active' : ''}" onclick="location.hash='#saboteurs/${id}'">${label}</div>`).join('')}</div>`;
