...
(function () {
  ...
  async function render(el) {
    ...
    // ── Сервер не видит адресов игроков ────────────────────────────
    // Тихая поломка: игра работает, а журнал входов, поиск мультоводов и
    // блокировка по адресу бесполезны. Место ей именно здесь — это то,
    // что надо ПОЧИНИТЬ, а не «информация к сведению».
    if (A2.can('security')) {
      try {
        const mc = await API.get('/api/admin/multi-check?min=99');
        if (mc.proxyBroken) {
          items.push({
            icon: '🌐', tone: 'warn',
            text: `Сервер <b>не видит адресов игроков</b>: у ${mc.blind} из ${mc.players} записан внутренний адрес`,
            when: 'журнал входов, поиск мультоводов и бан по адресу сейчас бесполезны',
            go: A2Router.build('tech', '', { p: 'checks' }), btn: 'Проверить',
          });
        }
      } catch (e) { /* необязательный блок — очередь не должна падать из-за него */ }
    }
