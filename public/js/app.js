...
const App = {
  ...
  async init() {
    ...
    // Раньше здесь стояло безусловное «нет игрока → на экран входа», и
    // адрес #verify/<код> затирался на #auth ещё до маршрутизации: игрок
    // жал кнопку в письме, попадал на форму входа и читал «подтвердите
    // почту». То есть подтверждение не срабатывало ровно у тех, ради
    // кого письмо и отправлено. В браузере, где игрок уже был внутри,
    ...
    // Клуб опрашивается с отступом: пока там пусто, чаще раза в полминуты
    // спрашивать незачем. Раньше запрос уходил каждые пять секунд у всех
    // и всегда — двенадцать запросов в минуту с каждого телефона, даже у
    // тех, кто в клуб не заходил ни разу.
    App._clubTick();
    App._clubSchedule();
  ...
  // ── СКОЛЬКО ЖИВЁТ ОТВЕТ /api/me ──────────────────────────────────
  // Переход между разделами устроен так: экран сначала ждёт /api/me и
  // только потом просит СВОИ данные. Две поездки на сервер подряд вместо
  // одной — а сервер отвечает за две миллисекунды, ждём мы сеть. На
  // телефоне это лишние 150–300 мс перед первой отрисовкой, на каждом
  // переходе.
  //
  // В пределах этого окна отдаём уже полученный ответ. Окно короткое и
  // сбрасывается ЛЮБЫМ действием игрока (см. API.req): списанное золото
  // и потраченная энергия обязаны появиться в шапке сразу, а не через
  // секунду. Всё, что меняется само (регенерация, доход), шапка и так
  // отсчитывает у себя каждую секунду.
  ME_TTL_MS: 2500,

  async pollMe(opts) {
    if (!API.token()) return;
    const force = !!(opts && opts.force);
    // Свежий ответ уже есть — второй поездки не делаем
    if (!force && App.me && App._meAt && Date.now() - App._meAt < App.ME_TTL_MS) return;
    // Запрос уже в пути: несколько экранов, стартовавших разом, ждут
    // один ответ, а не шлют одинаковые запросы каждый свой
    if (!force && App._mePending) return App._mePending;
    App._mePending = App._pollMeNow();
    try { await App._mePending; } finally { App._mePending = null; }
  },

  // Принудительное обновление: после действий игрока и там, где ответ
  // обязан быть свежим любой ценой.
  async _pollMeNow() {
    try {
  ...
  // Значок окна согласий. Свой, а не эмодзи: эмодзи рисуется шрифтом
  // системы и выглядит по-разному везде — где-то цветная бумажка,
  // где-то чёрный контур, а на части Android его нет вовсе и остаётся
  // пустой квадрат. Щит с галочкой читается одинаково и по смыслу
  // ближе: это про защиту прав, а не про бумажку.
  ICON_SHIELD: '<svg viewBox="0 0 24 24" aria-hidden="true">'
    + '<path d="M12 2.6 4.4 5.6v6c0 4.5 3.1 8.7 7.6 9.9 4.5-1.2 7.6-5.4 7.6-9.9v-6L12 2.6Z"/>'
    + '<path d="m8.6 12.1 2.4 2.4 4.4-4.6"/></svg>',
  ...
  showConsentGate(list) {
    ...
    box.innerHTML = `
      <div class="cg-box">
        <div class="cg-icon">${App.ICON_SHIELD}</div>
        <div class="cg-title">Нужно ваше подтверждение</div>
        <p class="cg-lead">Вы регистрировались до того, как в игре появились отдельные отметки
        о согласии. Спрашиваем прямо, а не считаем согласие данным по умолчанию.
        В игре ничего не меняется: прогресс, армия и альянс на месте.</p>

        <p class="cg-head-small">ОБЯЗАТЕЛЬНО</p>
        <div class="cg-req">
        ...
        <p class="cg-head-small">НЕОБЯЗАТЕЛЬНО — НА ИГРУ НЕ ВЛИЯЕТ</p>
        <div class="cg-opt">
          ${row('public', 'Показывать мой профиль другим игрокам', '/consent-public.html')}
          ${row('ads', 'Получать новости и акции', '/consent-ads.html')}
        </div>
  ...
  _STAFF_MARK: {
    // Владельца подписываем «Арбитром»: под этим именем его знают в
    // игре. Ключ остаётся owner — это идентификатор роли в поле игрока.
    owner:     { tag: 'арбитр',    title: 'Арбитр — владелец проекта' },
  ...
  // Отдельно от роли: сотрудник тоже может быть подписчиком, и одно не
  // заменяет другое.
  //
  // Здесь лежали ДВА объявления vipMark подряд, различавшиеся пробелом
  // перед значком. В объекте побеждает последнее, то есть работал
  // вариант с пробелом, а первый был мёртвым кодом — причём именно у
  // него стоял более подробный комментарий, так что правили бы, скорее
  // всего, именно его, и ничего бы не менялось.
  //
  // Пробел нужен: значок ставится сразу после имени, без него получалось
  // «БоецVIP».
  vipMark(isVip) {
  ...
  _clubBarKind: null,
  // Частота опроса клуба. Идёт партия или висит очередь — пять секунд:
  // ход в дуэли длится тридцать, и об истечении игрок должен узнать
  // заранее. Пусто два раза подряд — отступаем до тридцати секунд.
  CLUB_FAST_MS: 5000,
  CLUB_SLOW_MS: 30000,

  _clubSchedule() {
    clearTimeout(App._clubTimer);
    const fast = App._clubBarKind && App._clubBarKind !== 'none';
    App._clubTimer = setTimeout(async () => {
      if (!document.hidden) await App._clubTick();
      App._clubSchedule();
    }, fast ? App.CLUB_FAST_MS : App.CLUB_SLOW_MS);
  },

  // Игрок сам открыл клуб — возвращаемся к частому опросу немедленно,
  // не дожидаясь конца медленного круга.
  clubWake() {
    App._clubBarKind = App._clubBarKind || 'wake';
    App._clubTick().then(() => App._clubSchedule());
  },
  ...
  // Экраны зовут её на старте. Внутри — короткий кэш (ME_TTL_MS), любое
  // действие игрока его сбрасывает. Нужен ответ строго с сервера —
  // refreshMe(true).
  async refreshMe(force) { await App.pollMe(force ? { force: true } : undefined); },
  ...
  async route() {
    ...
    // Полоса закреплённой новости живёт вне экрана: она должна быть
    // видна везде, а не только там, где о ней вспомнили. Рисуется из
    // уже полученного /api/me, сети не трогает.
    App.renderPinnedNews();
    // Витрина акций НЕ здесь: её запрос уходил раньше запроса самого
    // экрана и на слабой связи приходил первым — игрок смотрел на
    // «Загрузка…» и список скидок под ней. Теперь она рисуется после
    // экрана и в простое (см. ниже).
    ...
    Promise.resolve(screen(c, param)).then(() => {
      ...
      if (preserve) {
        ...
        // Содержимое проявляется сверху вниз: на медленной связи
        // экран собирается кусками, и без этого он «дёргается»
        App._revealContent(c);
      }
      // Витрина акций — последней и только когда браузеру нечем
      // заняться: это справочная полоса внизу страницы, ради неё
      // нельзя занимать канал, пока грузится сам раздел
      App._whenIdle(() => App.renderSalesStrip());
    }).catch((e) => {
      c.innerHTML = `<div class="card err">${UI.esc(e.message)}</div>`;
    });
  },

  // Выполнить, когда браузер освободится. Нужен второстепенным вещам
  // (витрина акций): на слабой связи они не должны соперничать за
  // канал с тем, ради чего игрок открыл раздел.
  _whenIdle(fn) {
    const idle = window.requestIdleCallback || ((f) => setTimeout(f, 300));
    try { idle(fn, { timeout: 1500 }); } catch (e) { setTimeout(fn, 300); }
  },

  // Плавное проявление содержимого сверху вниз. Анимируем только
  // первые блоки: дальше игрок всё равно не видит, а лишние анимации
  // на слабом телефоне стоят дороже, чем дают.
  _revealContent(c) {
    if (!c || !c.children || !c.children.length) return;
    try {
      if (window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;
    } catch (e) {}
    const kids = Array.prototype.slice.call(c.children, 0, 8);
    kids.forEach((el, i) => {
      el.style.animationDelay = (i * 40) + 'ms';
      el.classList.add('rv-in');
      // Класс снимаем после анимации: иначе он переживает перерисовку
      // и следующий показ блока начинается с прозрачности
      setTimeout(() => { el.classList.remove('rv-in'); el.style.animationDelay = ''; }, 600 + i * 40);
    });
  },
  ...
  _sabNames: { ground: 'наземные', sea: 'морские', air: 'воздушные', building: 'подрывники', secret: 'секретные', suicide: 'смертники' },
  ...
  _showContainerResult(r) {
    ...
    // Диверсанты — своими картинками: по строке «секретные / подрывники»
  ...
  // Адреса, которые открывают из письма и без входа в игру. Держим
  // список в одном месте: он нужен и при загрузке, и в маршрутизаторе,
  // а разъехавшись, они дали бы ровно ту поломку, что чинили.
  _isMailLink() {
    return /^#?(verify|reset)\//.test(location.hash || '');
  },
