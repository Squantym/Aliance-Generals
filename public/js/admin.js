  ...
  _bindRoles(data, root, refresh) {
    ...
    const renderPerms = async () => {
      ...
      const DANGEROUS = {
        ...
        // банить. Спрашивали про баны и деньги, а про данные — нет.
        players: 'видеть досье любого игрока: почту, адреса, устройства и историю входов',
      };
...
proxy_set_header Host $host;</pre>
  ...
  // Выключатель, а не настройка: раздел наполнен и показан по умолчанию,
  // кнопка нужна, только чтобы временно его спрятать.
  async renderRefQuests(c) {
    ...
    c.innerHTML = `
      <div class="card">
        ...
        <p class="muted small">Кнопка «⚡ Операция „Пополнение“» в разделе «Пригласить друга». Задания настроены и показаны по умолчанию — выключайте, только если нужно временно убрать раздел у игроков.</p>
