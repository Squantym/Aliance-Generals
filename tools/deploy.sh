...
on_exit() {
  ...
  if [ $code -ne 0 ]; then
    ...
    # Раньше здесь стояло «снимите режим обслуживания». Скрипт его НЕ
    # включает — ни на входе, ни при отказе, — и совет отправлял искать
    # окно, которого нет. После того как одно такое окно провисело над
    # игрой двое суток, ложная подсказка про него особенно неуместна.
    echo "   Если вы закрывали игру вручную перед выкатом — она так и осталась"
    echo "   закрытой: откройте её кнопкой в панели."
...
echo ""
echo "   ⚠ Если вы закрывали игру перед выкатом — она ОСТАЁТСЯ закрытой."
echo "     Скрипт не открывает двери сам: после обновления игра может не"
echo "     подняться, и тогда о поломке сообщили бы игроки. Проверьте"
echo "     своими глазами и откройте кнопкой в панели."
set_state 'готово'
