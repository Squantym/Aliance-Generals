...
//
// Сервер ПЕРЕД запуском надо остановить (`pm2 stop generals-test`). Он
// держит игроков в памяти и при остановке пишет её на диск целиком —
// аккаунт, заведённый скриптом на живой базе, был бы затёрт этой
// записью. Плюс замок базы всё равно не пустит второй процесс.
//
// Запуск (в папке тестовой установки):
...
(async () => {
  ...
  db.markUser(made.id);
  // Именно flushAllNow, а не save(): save лишь помечает данные грязными и
  // ставит сброс через 400 мс, а следом идёт process.exit(0) — таймер не
  // успевает сработать НИ РАЗУ, и аккаунт умирает вместе с процессом.
  // Скрипт при этом печатал «создан», сервер потом поднимался с нулём
  // игроков, и понять, почему в пустой мир не пускает, было нечем.
