...
function registerRoutes(app: any) {
  ...
  app.add('GET', '/api/world', () => ({
    ...
    test: testWorld.view(),
    // Открыта ли регистрация и, если нет, почему.
    //
    // Появилось после суток поисков. Регистрация была закрыта правилом
    // «нет почты — не заводим аккаунты», но узнать об этом можно было
    // только попытавшись зарегистрироваться либо зайдя в панель. Со
    // стороны это выглядело как «игра сломалась после обнуления», и
    // причину искали не там, где она была.
    //
    // Теперь состояние видно одним запросом, без входа: и человеку на
    // экране регистрации, и тому, кто разбирается с сервером.
    registration: (() => {
      const why = maintenance.isOn()
        ? maintenance.view().reason + ' Регистрация ненадолго закрыта.'
        : testWorld.isOn()
          ? 'Это тестовый мир. Регистрация закрыта — аккаунт выдаёт администратор.'
          : (() => { try { return require('./services/email').registrationBlocked(); } catch (e) { return ''; } })();
      return { open: !why, why };
    })(),
  }), { open: true });
  ...
  app.add('GET', '/api/me', (req) => {
    ...
    return {
      ...
      //
      // Здесь стояло `process.env.ADMIN_PATH || null`, и при НЕзаданном
      // ADMIN_PATH адрес приходил пустым — а блок в настройках рисуется
      // только когда он не пуст. То есть кнопки «Открыть панель» не было
      // ВООБЩЕ, хотя сама панель работала: запасной путь /admin описан в
      // core/http.ts, а сюда его не протянули. Владелец искал панель
      // руками и попадал в старую.
      //
      // Панель теперь одна: по голому адресу отдаётся она же. Старая
      // со строкой вкладок убрана целиком — две панели над одними и
      // теми же экранами расходились, и сотрудник видел разный штаб
      // в зависимости от того, по какой ссылке зашёл.
      staffPanel: (() => {
        if (!require('./services/roles').zonesFor(req.user).length) return null;
        // Нормализуем так же, как core/http.ts: принимаем «shtab-x7»,
        // «/shtab-x7» и вариант с косой чертой в конце.
        let p = String(process.env.ADMIN_PATH || '').trim();
        if (p && !p.startsWith('/')) p = '/' + p;
        if (p.length > 1 && p.endsWith('/')) p = p.slice(0, -1);
        // Хвост /v2 не нужен: панель теперь одна, и голый адрес ведёт
        // в неё же. Старые ссылки с /v2 продолжают работать.
        return p || '/admin';
      })(),
  ...
  // Пригласить можно и по позывному (вкладка альянса), и по id (кнопка
  // в профиле игрока) — сервису всё равно, чем его позвали.
  app.add('POST', '/api/alliance/invite',   act((req, n) => palliance.invitePlayer(req.user, req.body.name, n, req.body.targetId)));
  ...
  // Админские ручки легиона живут не под /api/admin, но контроль у них
  // тот же: зона «Легионы» (см. ZONE_RULES).
  app.add('POST', '/api/legion/admin-join',        act((req, n) => require('./services/groups').adminJoin(req.user, 'legion', req.body.legionId, n)), { admin: true });
  app.add('POST', '/api/legion/admin-deposit',     act((req, n) => legion.adminDeposit(req.user, req.body.legionId, req.body.amount, n, req.body.resource)), { admin: true });
  ...
  // { admin: true } — запрос проходит через общий контроль зон (зона
  // «Новости»), а сам сервис проверяет её ещё раз: попытка обойти адрес
  // должна упираться в оба замка, а не в один.
  app.add('POST', '/api/news/create', act((req, n) => require('./services/news').create(req.user, req.body, n)), { admin: true });
