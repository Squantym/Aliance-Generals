...
// ---------- ИГРОК ----------
// Главная сущность. ВНИМАНИЕ к двум разным «гербам» (поля названы по
// прежней механике (наследие), переименовывать их в живой базе нельзя):
//   ears       — ТРОФЕЙНЫЕ гербы (сорванные у врагов): ресурс для трат
//                 и для зала славы. Растёт при срыве чужого герба.
//   crestParts — части СВОЕГО герба (лимит CREST.PARTS): их срывают
//                 враги, они восстанавливаются. Это НЕ ресурс для трат.
...
export interface User {
  ...
  crestParts: number;   // части своего герба (лимит), восстанавливаются
  crestLostAt: number[];
  // Кто сорвал части герба: crestTakers[0] = первая сорванная,
  // crestTakers[1] = вторая. Каждый элемент: { id, name } или null.
  crestTakers?: ({ id: string; name: string } | null)[];
  // Послание от того, кто сорвал весь герб (видно всем в профиле)
  ...
  // Рейтинговые сезоны
  seasonRating?: number;
  rating?: number;       // накопительный рейтинг: победы/поражения/гербы/жетоны/мины
