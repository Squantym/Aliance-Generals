...
// Снимок ВМЕСТЕ с именем коллекции, которой он принадлежит.
// Нужен потому, что откат раньше брал снимок по одному seq и писал его
// в ту коллекцию, которую назвал вызывающий. Совпадение никто не сверял:
// снимок мира можно было записать в рынок и испортить обе коллекции
// одним запросом. Из панели так не нажать — она передаёт пару из одной
// строки, — но API принимал любую комбинацию.
function snapshotGetFull(seq: number): any | null {
  const r = db.prepare('SELECT collection, at, label, data FROM snapshots WHERE seq = ?').get(seq) as any;
  if (!r) return null;
  try {
    return { seq, collection: r.collection, at: r.at, label: r.label, data: JSON.parse(r.data) };
  } catch (e) { return null; }
}
