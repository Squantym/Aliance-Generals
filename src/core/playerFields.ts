...
const PLAYER_FIELDS: Record<string, FieldOwner> = {
  ...
  // Согласия игрока: что принял, какой редакции, когда, с какого адреса
  // и когда отозвал. Отозванное не удаляется — сам факт прошлого
  // согласия доказывает законность обработки до отзыва.
  consents:           { owner: 'consent',    note: 'согласия: документ, версия, время, IP, отзыв' },
  // Ключ ссылки «отписаться» в письмах рассылки. Отдельный от сессии:
  // ссылка живёт в письме годами, а сессия — тридцать дней.
  mailKey:            { owner: 'consent',    note: 'ключ ссылки отписки от рассылки' },
  ...
  // Сколько из нынешнего баланса куплено за деньги. Нужно, чтобы при
