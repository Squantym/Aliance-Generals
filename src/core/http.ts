...
interface RouteOpts {
  ...
  // Маршрут НЕ пишется в журнал действий. Ставится осознанно и только
  // там, где сам смысл ручки — отсутствие следа (тихая выдача
  // владельца). Права такой маршрут обязан проверять сам: журнала,
  // по которому потом можно разобраться, у него нет.
  noLog?: boolean;
}
...
function createApp() {
  ...
  const app = {
    ...
    listen(port: number, cb?: () => void) {
      const server = http.createServer(async (req, res) => {
        ...
        try {
          ...
          if (!found.opts.open) {
            ...
            // ── Позывной сброшен, игра закрыта до смены ──────────
            // Закрывает сервер, а не окно в браузере: запрос мимо интерфейса
            // иначе проходил бы. Смотреть можно, действовать — только сменить
            // позывной (список разрешённого — services/nameReset.ts).
            try {
              if (require('../services/nameReset').blocks(user, req.method, pathname)) {
                return sendJson(res, 403, {
                  error: 'Сначала смените позывной: его сбросила модерация.', nameResetBlock: true,
                }, acceptEncoding);
              }
            } catch (e) { /* модуль недоступен — не запираем игру */ }
            // ── Режим обслуживания ──────────────────────────────
          ...
          if (reqCtx.user && req.method === 'POST' && !found.opts.noLog) {
            auditLog.record({
      ...
      // Слушаем ТОЛЬКО внутренний адрес. Снаружи игру отдаёт nginx по
      // HTTPS и сам обращается к ней на 127.0.0.1. Раньше здесь стоял
      // 0.0.0.0, и порты 3000/3001 были открыты в интернет напрямую, без
      // шифрования: пароли, в том числе от админ-панели, уходили бы
      // открытым текстом любому, кто зашёл по IP с номером порта.
      // Проверено снаружи 10.09.2026 — оба мира отвечали 200.
      // Межсетевой экран на сервере это закрывает, но отключить его —
      // одна команда; закрытый адрес в коде не зависит от настройки сервера.
      // LISTEN_HOST=0.0.0.0 — только осознанно: разработка с телефона по
      // локальной сети или сервер без nginx перед игрой.
      const host = String(process.env.LISTEN_HOST || '').trim() || '127.0.0.1';
