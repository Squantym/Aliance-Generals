// ===================================================================
// src/services/silos.js — раздел «Ракетные шахты» в Производстве
//
// Механика:
...
function boost(user, siloId, notices) {
...
function fuelReady(user, siloId, amount, notices) {
  const silo = silos(user).find((s) => s.id === siloId);
  if (!silo) throw new u.ApiError('Шахта не найдена');
  const r = silo.rocket;
...
function fuelPower(user, siloId, amount, notices) {
  const silo = silos(user).find((s) => s.id === siloId);
  if (!silo) throw new u.ApiError('Шахта не найдена');
  const r = silo.rocket;
  if (r.buildFinishesAt > Date.now()) throw new u.ApiError('Шахта ещё строится');
...
function launch(user, siloId, targetId, notices) {
  const silo = silos(user).find((s) => s.id === siloId);
  if (!silo) throw new u.ApiError('Шахта не найдена');
  const r = silo.rocket;
  if (r.buildFinishesAt > Date.now()) throw new u.ApiError('Шахта ещё строится');
  ...
  for (const [id, count] of Object.entries(target.buildings || {})) {
    ...
    if (def && count > 0) {
      for (let i = 0; i < count; i++) buildingEntries.push(id);
    }
  }
...
module.exports = { view, build, boost, fuelReady, fuelPower, launch };
