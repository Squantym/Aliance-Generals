...
// Какая комната запрошена. Общий эфир и доски («Набор в альянс»,
// «Набор в легион», «Вопросы новичков») читают все, чат альянса — только
// взаимные союзники автора. Идентификатор 'recruit' оставлен прежним:
// в нём уже лежит история «Позывных», переименовано только название.
const ROOMS = ['global', 'recruit', 'legion', 'newbie', 'alliance'];
function roomOf(room?: string): string {
  return ROOMS.indexOf(String(room || '')) >= 0 ? String(room) : 'global';
}
...
function chatPost(user: User, text: string, room?: string) {
  ...
  user.lastChatAt = now;
  const w = world();
  // Комната: 'global' — общий эфир, 'recruit' — «Набор в альянс»,
  // 'legion' — «Набор в легион», 'newbie' — «Вопросы новичков».
  // Разделение нужно, чтобы объявления и вопросы не тонули в живом чате,
  // а чат не превращался в ленту объявлений.
