// ===================================================================
// src/services/production.ts — раздел «Производство» (с 70 уровня)
...
function baseModernPricePerUnit(cu: any, toMk: number): number {
...
// Купить ещё один цех
function buyWorkshop(user: User, notices: Notices) {
  checkUnlocked(user);
...
function startModernization(user: User, body: any, notices: Notices) {
  checkUnlocked(user);
...
function boostProcess(user: User, processId: string, notices: Notices) {
  if (!user.modernQueue) user.modernQueue = [];
...
export = { view, buyWorkshop, startModernization, boostProcess, nextWorkshopPrice };
