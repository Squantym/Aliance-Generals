...
// ---------- Цена следующего уровня боевой постройки (КМ + ресурс) ----------
function battleBuildingCost(b, currentLevel) {
  // currentLevel — текущий уровень; возвращаем стоимость перехода на currentLevel+1
  const growth = b.priceGrowth || config.LEGION.BATTLE_BUILDING_PRICE_GROWTH_DEFAULT;
  const kmarks = Math.round(b.priceBase * Math.pow(growth, currentLevel));

  let ears = 0, tokens = 0;
  if (b.resource === 'ear' || b.resource === 'both') {
    ears = Math.round(b.earBase * Math.pow(b.earGrowth, currentLevel));
  }
  if (b.resource === 'token' || b.resource === 'both') {
    tokens = Math.round(b.tokenBase * Math.pow(b.tokenGrowth, currentLevel));
  }
  return { kmarks, ears, tokens };
}
...
function view(user) {
  ...
  const techs = config.LEGION_TECHS.map((t) => {
    ...
    const canLearn = nextLevelData && l.ratingPoints >= nextLevelData.ratingReq && !l.techQueue;
    return {
...
// ===================================================================
// ОБМЕН ДОЛЛАРОВ НА КЛАНМАРКИ (Стальные марки)
// 1 000 $ = 1 КМ. Обменивает из личного кошелька в казну легиона.
// ===================================================================
function exchangeToKmarks(user, dollars, notices) {
  const l = legionOf(user);
  if (!l) throw new u.ApiError('Вы не состоите в легионе');
  ensureLegionFields(l);
  const amt = u.toInt(dollars, 0);
  if (amt <= 0) throw new u.ApiError('Укажите сумму обмена');
  if (amt % config.LEGION.KMARK_EXCHANGE_RATE !== 0) {
    throw new u.ApiError(`Сумма должна быть кратна ${u.fmt(config.LEGION.KMARK_EXCHANGE_RATE)} (1 КМ)`);
  }
  if (user.dollars < amt) throw new u.ApiError('Не хватает долларов');
  const km = amt / config.LEGION.KMARK_EXCHANGE_RATE;
  user.dollars -= amt;
  l.kmarks += km;
  db.save('legions');
  notices.push(`💱 Обменяно: $${u.fmt(amt)} → ${u.fmt(km)} КМ. В казне легиона: ${u.fmt(l.kmarks)} КМ.`);
  return { kmarks: l.kmarks };
}
...
// ===================================================================
// БОЕВЫЕ ПОСТРОЙКИ (КМ + уши/жетоны из казначейства)
// ===================================================================
function buildBattle(user, buildingId, notices) {
  ...
  // Проверяем КМ в казне
  if (l.kmarks < cost.kmarks) {
    throw new u.ApiError(`Не хватает Стальных марок (нужно ${u.fmt(cost.kmarks)} КМ, есть ${u.fmt(l.kmarks)} КМ)`);
  }
  // Проверяем уши у лидера (берём из личного инвентаря лидера)
  if (cost.ears > 0 && (user.earsCurrent || 0) < cost.ears) {
    throw new u.ApiError(`Не хватает ушей (нужно ${cost.ears}, у вас ${user.earsCurrent || 0})`);
  }
  // Проверяем жетоны у лидера
  if (cost.tokens > 0 && (user.tokens || 0) < cost.tokens) {
    throw new u.ApiError(`Не хватает жетонов (нужно ${cost.tokens}, у вас ${user.tokens || 0})`);
  }

  // Списываем ресурсы
  l.kmarks -= cost.kmarks;
  if (cost.ears > 0)   user.earsCurrent -= cost.ears;
  if (cost.tokens > 0) user.tokens = (user.tokens || 0) - cost.tokens;

  l.battleBuildings[b.id] = lvl + 1;
  db.save('legions');

  const newLvl = lvl + 1;
  notices.push(`⚔️ «${b.name}» улучшена до ур. ${newLvl}! Бонус: ${newLvl * b.perLvl}${b.bonusUnit || '%'}.`);
  ...
  for (const memberId of l.members) {
    if (memberId !== user.id) {
      const m = users[memberId];
      if (m) notif.push(m.id, 'legion_build',
        `🏗 Новая боевая постройка легиона`,
        { buildingName: b.name, level: newLvl });
    }
  }
...
function startTech(user, techId, notices) {
  ...
  // Требования рейтинга
  if (l.ratingPoints < levelData.ratingReq) {
    throw new u.ApiError(`Нужно ${u.fmt(levelData.ratingReq)} очков рейтинга клана (есть ${u.fmt(l.ratingPoints)})`);
  }
  // КМ
  if (l.kmarks < levelData.priceKmarks) {
    throw new u.ApiError(`Не хватает КМ (нужно ${u.fmt(levelData.priceKmarks)}, есть ${u.fmt(l.kmarks)})`);
  }
  // Уши лидера
  ...
  // Списываем
  l.kmarks -= levelData.priceKmarks;
...
function resolveActiveBattle(l, all, users) {
  ...
  const loot = Math.floor((loser.kmarks || 0) * 0.20); // 20% кланмарок победителю
  ...
  const msgW = `🏆 ПОБЕДА в бою легионов! Захвачено ${u.fmt(loot)} КМ + ${config.LEGION.BATTLE_LOOT_KMARKS} КМ.`;
  const msgL = `💀 ПОРАЖЕНИЕ в бою легионов. Потеряно ${u.fmt(loot)} КМ.`;
...
module.exports = {
  ...
  exchangeToKmarks, buildBattle, depositResources,
  startTech, resolveTechQueue,
  shopBuy, gearPick,
  challengeLegion, acceptChallenge, declineChallenge,
  // Делегируем бой в legionBattle
