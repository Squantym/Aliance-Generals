// ===================================================================
// src/services/legion.ts — Легион (клан): казна, постройки, войны,
...
function legionOf(user: User): any { return user.legionId ? legions()[user.legionId] || null : null; }
...
function nextBuildingPrice(building: any, level: number): number {
...
function techDurationMs(tech: any, level: number): number {
...
// ===================================================================
function legionWarPower(legion: any): number {
...
// ===================================================================
function exchangeToReserves(user: User, dollars: number, notices: Notices) {
  const l = legionOf(user);
  if (!l) throw new u.ApiError('Вы не состоите в легионе');
  ensureLegionFields(l);
...
// ===================================================================
function build(user: User, buildingId: string, notices: Notices) {
  const l = legionOf(user);
  if (!l) throw new u.ApiError('Вы не состоите в легионе');
...
// ===================================================================
function startTech(user: User, techId: string, notices: Notices) {
  const l = legionOf(user);
  if (!l) throw new u.ApiError('Вы не состоите в легионе');
...
// ===================================================================
function resolveTechQueue(): void {
  const all = legions();
  let changed = false;
...
// ===================================================================
function shopBuy(user: User, itemId: string, qty: number, notices: Notices) {
  const l = legionOf(user);
  if (!l) throw new u.ApiError('Вы не состоите в легионе');
...
// ===================================================================
function gearPick(user: User, itemId: string, notices: Notices) {
  const l = legionOf(user);
  if (!l) throw new u.ApiError('Вы не состоите в легионе');
  ensureLegionFields(l);
...
// ===================================================================
function challengeLegion(user: User, enemyId: string, notices: Notices) {
  const l = legionOf(user);
  if (!l) throw new u.ApiError('Вы не состоите в легионе');
...
// ПРИНЯТИЕ ВЫЗОВА
// ===================================================================
function acceptChallenge(user: User, notices: Notices) {
  const l = legionOf(user);
  if (!l) throw new u.ApiError('Вы не состоите в легионе');
...
// ОТКЛОНИТЬ ВЫЗОВ
// ===================================================================
function declineChallenge(user: User, notices: Notices) {
  const l = legionOf(user);
  if (!l) throw new u.ApiError('Вы не состоите в легионе');
...
// ===================================================================
function declareWar(user: User, enemyId: string, notices: Notices) {
  const l = legionOf(user);
  if (!l) throw new u.ApiError('Вы не состоите в легионе');
...
// ЛИМИТ УЧАСТНИКОВ
// ===================================================================
function memberLimit(l: any): number {
...
// ЗВАНИЯ УЧАСТНИКОВ
// ===================================================================
function getMemberRank(l: any, userId: string): number {
  if (l.leaderId === userId) return config.LEGION.RANK_GENERAL;
  return (l.memberRanks || {})[userId] || 0;
}

function setRank(user: User, targetId: string, rankIndex: number, notices: Notices) {
  const l = legionOf(user);
  if (!l) throw new u.ApiError('Вы не состоите в легионе');
  ensureLegionFields(l);
...
// ЧАТ ЛЕГИОНА
// ===================================================================
function chatGet(user: User): any {
  const l = legionOf(user);
  if (!l) throw new u.ApiError('Вы не состоите в легионе');
  ensureLegionFields(l);
