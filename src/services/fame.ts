// ===================================================================
// src/services/fame.ts — Зал славы
//
// Архитектура:
//   - «За всё время» = текущие накопленные счётчики игроков
//   - «Сегодня» = текущее_значение − снапшот_начала_дня
//
// Снапшот хранится в коллекции 'dailyFame' (в своей базе — строка
// в таблице collections, в json-режиме — файл dailyFame.json).
...
function loadSnap(): any {
  return db.load<any>('dailyFame', {});
}

function saveSnap(snapshotDate: string, snapshot: any): void {
  const store = db.load<any>('dailyFame', {});
...
function ensureSnapshotUpToDate(): any {
...
// ── Топ-10 по значению ────────────────────────────────────────────
function buildTop(valFn: (u: User) => number): any[] {
  return Object.values(player.users() as Record<string, User>)
    .map((p) => ({
...
function forceResetSnapshot(): void {
  const store = db.load<any>('dailyFame', {});
  store.snapshotDate = null;
  store.snapshot     = {};
  db.save('dailyFame');
...
export = { fame, resetDailyIfNeeded, forceResetSnapshot };
