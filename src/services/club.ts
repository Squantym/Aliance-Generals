...
function payout(user: User, game: string, want: number, notices: Notices): number {
  ...
  // Выплата = выигранная партия. Считаем её здесь, в единственном месте
  // выдачи: условия приглашений спрашивают «сколько игр выиграл», и
  // отдельный счётчик в каждой игре однажды забыли бы поставить.
  // Считаем по ЗАПРОШЕННОЙ сумме: партия выиграна и тогда, когда золото
  // срезал суточный потолок.
  if (wanted > 0) {
    try { require('./dailyQuests').bump(user, 'clubWon', 1); } catch (e) {}
  }
