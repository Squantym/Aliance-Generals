// ===================================================================
// src/services/mines.ts — раздел «Шахты» в Производстве
//
// Механика:
...
function mines(user: User): any[] {
  // user.mines — рантайм-поле (массив шахт)
  if (!user.mines) user.mines = [];
  return user.mines;
}

// Текущий "номер шахты" игрока (для расчёта цены — каждая следующая дороже)
function nextMineCost(user: User): number {
...
function nextMineDollars(user: User): number {
...
function rollInitialGold(): number {
...
function ensureDailyMinutes(mine: any): void {
...
function descend(user: User, mineId: string, minutes: number, notices: Notices) {
...
function fightTerrorists(user: User, mineId: string, notices: Notices) {
  const mine = mines(user).find((m) => m.id === mineId);
  if (!mine) throw new u.ApiError('Шахта не найдена');
...
function collectGold(user: User, mineId: string, notices: Notices) {
  const mine = mines(user).find((m) => m.id === mineId);
  if (!mine) throw new u.ApiError('Шахта не найдена');
...
function rebuild(user: User, mineId: string, notices: Notices) {
  const mine = mines(user).find((m) => m.id === mineId);
  if (!mine) throw new u.ApiError('Шахта не найдена');
...
export = { view, build, rebuild, descend, fightTerrorists, collectGold, refreshAll };
