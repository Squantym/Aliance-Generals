...
function refresh(user: User): void {
  ...
  //
  // У живых игроков в базе лежат ПРЕЖНИЕ поля — уши (наследие). Молча читать
  ...
  //
  // Соответствие: прежняя пара превращается в три части герба ПО ДОЛЕ
  // сохранности, а не «сколько было, столько и стало». Игрок, у которого
  // уцелела одна половина из двух, был повреждён наполовину — и остаётся
  // повреждён наполовину: полторы части округляем вверх, в пользу игрока.
...
//   у тебя сорвали часть герба −3; подорвался на мине −3.
...
function publicProfile(target: User, viewer: User): any {
  ...
  return {
    ...
    // были жёстко «левое» и «правое» — под прежнюю пару; частей три, и такой
...
// Восстановить одну часть герба мгновенно за золото (если сорвана хотя бы одна)
function restoreEar(user: User, notices: Notices) {
  if (user.crestParts >= config.CREST.PARTS) {
    throw new u.ApiError('Ваш герб целый — восстанавливать нечего');
  }
  ...
  user.crestParts = Math.min(config.CREST.PARTS, user.crestParts + 1);
  // Убираем самую старую запись о потере (эта часть уже восстановлена)
  if (user.crestLostAt.length > 0) user.crestLostAt.shift();
  // Если часть вернулась — штраф пересчитывается сам
  // Штраф отдельно снимать не нужно: он считается от числа частей.
  // Если герб собран целиком — стираем записи о том, кто рвал, и послание
  if (user.crestParts >= config.CREST.PARTS) {
    user.crestTakers = [null, null];
    user.crestMessage = null;
  }
  notices.push(`🛡 Часть герба восстановлена за 🪙 ${config.CREST.RESTORE_GOLD}. Частей на месте: ${user.crestParts}/${config.CREST.PARTS}.`);
