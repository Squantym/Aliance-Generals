...
// ---------------------------------------------------------------------
// Урон считается от ДОЛИ атаки нападающего в защите обороняющегося:
// share = атака / защита. Числа — решение владельца (18.09.2026) после
// жалобы игрока: защита 21 млн против атаки 3 млн, а нападающий всё
// равно пробивал и убивал. Раньше «почти бессильный» атакующий всё
// равно бил на 3–5 за удар, и серия ударов добивала кого угодно.
//
//   доля < 40%        -> 1–5    (атака заметно ниже защиты)
//   40% ≤ доля < 70%  -> 5–10
//   70% ≤ доля < 85%  -> 10–15
//   доля ≥ 85%        -> 30–36  — полный удар; ВЫШЕ НЕ РАСТЁТ, даже если
//                                 атака вдвое больше защиты
//
// Крит остаётся множителем поверх: ×2, с трофеем до ×6 (потолок 216).
// ---------------------------------------------------------------------
function resolveDamage(atk: number, def: number): any {
  const share = Math.max(0, atk) / Math.max(1, def);   // доля атаки в защите
  let dealt;

  if (share < 0.40)      dealt = u.rnd(1, 5);
  else if (share < 0.70) dealt = u.rnd(5, 10);
  else if (share < 0.85) dealt = u.rnd(10, 15);
  else                   dealt = u.rnd(30, 36);        // полный удар

  dealt = u.clamp(Math.round(dealt), 1, B.MAX_HIT);
  return { dealt };
}
