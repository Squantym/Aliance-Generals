...
function newUser(id: string, name: string, email_: string, passHash: string, salt: string, country: string, isAdmin: boolean, emailVerified: boolean, gender?: string): User {
  ...
  return {
    ...
    ears: 0, tokens: 0, crestPartsLost: 0,
    crestParts: config.CREST.PARTS, crestLostAt: [],
    // Слотов ровно столько, сколько частей: список короче молча терял бы
    // отметку о том, кто снял последнюю.
    crestTakers: new Array(config.CREST.PARTS).fill(null), crestMessage: null,
    battle: { attacks: 0, wins: 0, losses: 0, defWins: 0, defLosses: 0, breaches: 0 },
    counters: { wins: 0, attacks: 0, breaches: 0, unitsBought: 0, buildingsBuilt: 0, missionStages: 0, crestsTorn: 0, moneyEarned: 0, battleLoot: 0, level: 1 },
    achStages: {},
    missions: {},
