...
function run(actor: any, opts: Opts = {}) {
  ...
  //    Срок НЕ называем. Раньше стояло 60 минут, и игроки видели
  //    «ориентировочно до ...», хотя turnOn самооткрытие не ставит
  //    никогда — мир открывает человек, убедившись, что тот поднялся
  //    чистым. После обнуления 30.08 окно провисело так двое суток, а
  //    обещанный срок истёк на первом часе. Без срока клиент честно
  //    пишет «бессрочно» и никого не обманывает.
  try {
    require('./maintenance').turnOn(
      String((actor && actor.name) || ''),
      'Игра перезапускается с нуля. Скоро откроем новый мир.');
