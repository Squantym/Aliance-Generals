// ===================================================================
// src/services/antibot.js — мягкая защита от скриптов/кликеров/ботов
//
...
function track(user, actionType) {
  const b = ensure(user);
  applyDecay(b);
...
function passVerification(user) {
  const b = ensure(user);
  b.suspicion = 0;
...
function needsVerification(user) {
  const b = ensure(user);
  applyDecay(b);
  return b.verifyRequired && b.suspicion >= VERIFY_THRESHOLD;
}

module.exports = { track, passVerification, needsVerification, ensure };
