...
type Zone =
  ...
  | 'season';     // настройки сезона, досрочное завершение сезона
...
// усмотрение, включая опасные разделы: при их включении панель выводит
// подтверждение и прямо называет, что именно роль сможет делать
// (см. DANGEROUS в public/js/admin.js).
//
// Выше здесь лежал комментарий от прежней версии: «зоны, недоступные
// роли „Дозор“ ни при каких настройках». Это перестало быть правдой —
// теперь запрещена только база данных, а «Модерацию» владелец выдать
// может. Комментарий описывал защиту, которой уже нет, и читающий код
// сделал бы неверный вывод о том, что модератор гарантированно не
// удалит аккаунт.
...
const ZONE_RULES: Array<[RegExp, Zone]> = [
  ...
  // Праздники — расписание акций. Зона «Акции»: то же, что скидки,
  // только включается по календарю
  [/^\/api\/admin\/holidays?(\/save|\/delete|\/toggle)?$/, 'discounts'],
  ...
  // попала бы в зону «Ресурсы», которую владелец выдаёт администраторам.
  ...
  [/^\/api\/admin\/(net-bans|net-ban-preview|net-ban|net-unban)$/, 'moderation'],
  // Платежи покупателей — владельческий раздел (проверка isOwner в сервисе)
  [/^\/api\/admin\/payments(\/|$)/,                 'roles'],
  // Состояние подключения кассы — тот же владельческий раздел
  [/^\/api\/admin\/payments-provider$/,             'roles'],
  // Lava Top: связка товаров игры с каталогом кассы. Там же ключи и
  // деньги покупателей — владельческий раздел (isOwner в сервисе)
  [/^\/api\/admin\/lavatop(\/|$)/,                  'roles'],
  // Письма и рассылка. Зона «Роли» выбрана намеренно: её имеет только
  // владелец, а письмо от имени игры — его голос, а не сотрудника.
  // Сам сервис дополнительно проверяет isOwner.
  [/^\/api\/admin\/mail\//,                         'roles'],
  ...
  [/^\/api\/news\/(create|update|delete|pin|image)$/, 'news'],
