...
//   подрывники → мощь оборонительных построек противника
...
//     у ЦЕЛИ удара).
//   - секретные/подрывники: гибнут ТОЛЬКО «за компанию» с обычными —
//     на каждые 5 уничтоженных обычных 1 секретный И 1 подрывник
...
function destroyRegular(user: User, count: number, notices: Notices | null): Record<string, number> {
  ...
  // пересечении кратного 5 гибнет 1 секретный И 1 подрывник (если есть)
  if (totalLost > 0) {
...
function rocketDestroy(target: User, powerFrac: number, notices: Notices): Record<string, number> {
  ...
  // (1 секретный + 1 подрывник на каждые 5 обычных). Значит суммарно выйдет
