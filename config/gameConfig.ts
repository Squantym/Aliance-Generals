...
const CLUB = {
  ...
  DAILY_GOLD_CAP: 300,
  // Общий кулдаун на ВСЕ игры после любого выигрыша. Один на клуб, а не
  // на игру: иначе игрок просто перебирает пять игр по кругу и никакого
  // перерыва нет.
  SHARED_CD_MIN: 10,
  ...
  // ── 3. НОЧНОЙ КАРАВАН ─────────────────────────────────────────
  //    Единственная игра клуба, где противник — ЖИВЫЕ ИГРОКИ, а не
  //    сервер. Ты ведёшь караван по одному из трёх маршрутов, а на
  //    маршрутах ждут засады, оставленные другими. Прошёл — добыча;
  //    напоролся — караван потерян, а золото уходит тому, кто эту
  //    засаду поставил.
  //
  //    Почему именно так: скрипт не может предсказать, куда пойдут
  //    люди. Здесь нет ни шаблона сервера, который можно вычитать, ни
  //    оптимальной стратегии — есть только чужие решения, принятые
  //    минуту назад. Это самая устойчивая к автоматизации конструкция
  //    из всех, что есть в клубе.
  CONVOY_ROUTES: [
    { id: 'mountain', name: 'Горный перевал', icon: '⛰' },
    { id: 'coast',    name: 'Прибрежное шоссе', icon: '🌊' },
    { id: 'steppe',   name: 'Степной тракт', icon: '🌾' },
  ],
  CONVOY_LOOT: 8,             // прошёл маршрут
  CONVOY_AMBUSH_PAY: 10,      // твоя засада сработала (приходит владельцу)
  CONVOY_MAX_AMBUSHES: 30,    // сколько засад держим в мире
  CONVOY_LOG: 5,              // сколько последних событий видно всем
  CONVOY_CD_MIN: 8,

  // ── 4. САПЁРНАЯ ТРОПА ─────────────────────────────────────────
  //    Поле 3×3, в нём две мины. Открываешь клетки по одной, добыча
  //    растёт, остановиться можно в любой момент. Наступил — потерял
  //    всё набранное.
  //
  //    От «идти до конца» удерживает арифметика, а не окрик: с каждой
  //    открытой клеткой мины занимают всё большую долю оставшихся.
  //    Числа подобраны так, чтобы лучшая остановка была в середине —
  //    проверено симуляцией в тесте.
  SAPPER_CELLS: 9,
  SAPPER_MINES: 2,
  SAPPER_LOOT: [2, 5, 9, 14, 20, 27, 35],   // добыча за 1..7 открытых клеток
  SAPPER_CD_MIN: 8,

  // ── 5. ПОЛЕВОЙ ТОТАЛИЗАТОР ────────────────────────────────────
  //    Единственная игра клуба, кроме преферанса, где платят ЗА ВХОД.
  //    Три отделения бегут учения, у каждого свой шанс и свой
  //    коэффициент. Ставка фиксированная.
  //
  //    Коэффициенты занижены относительно честных на BOOKIE_MARGIN:
  //    ожидание отрицательное при ЛЮБОЙ ставке, и это не жадность, а
  //    защита. Скрипт, играющий круглые сутки, здесь не богатеет, а
  //    разоряется — арифметику не обойти ни ботом, ни удачей вдолгую.
  BOOKIE_STAKE: 10,
  BOOKIE_MARGIN: 0.15,        // 15% в пользу заведения
  BOOKIE_SQUADS: [
    { id: 'alpha',  name: 'Отделение «Альфа»',  icon: '🔴', chance: 50 },
    { id: 'bravo',  name: 'Отделение «Браво»',  icon: '🔵', chance: 30 },
    { id: 'charlie', name: 'Отделение «Чарли»', icon: '🟢', chance: 20 },
  ],
  BOOKIE_CD_MIN: 6,
