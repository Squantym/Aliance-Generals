...
const CLUB = {
  ...
  // ── 3. Ночной рейд (пройти рубежи и вовремя отойти) ──
  //    Игра про остановку, а не про угадывание: добыча растёт быстро,
  //    но риск растёт быстрее. Вероятности показаны игроку честно —
  //    решение осознанное. Лучшая остановка по расчёту — третий рубеж
  //    (ожидание 5.1 🪙); жадность до шестого даёт 2.6.
  RAID_RISK_PCT: [12, 20, 28, 36, 45, 55],   // риск сорваться на рубеже N
  RAID_LOOT:     [3,  6,  10, 15, 22, 32],   // добыча, если отойти после рубежа N
  RAID_CD_WIN_MIN: 12, RAID_CD_FAIL_MIN: 6,
